...
export default function AlertMessage(props) {
  ...
  messages["send-transaction"] = {
    ...
    "description": "Transaction failed.  Common causes include an insufficient balance to complete the transaction, invalid transaction parameters, or user account lacking necessary authorization."
  };
  ...
  return (
    <Box border="1px solid" width="50vw" id={`alert-${type}`} backgroundColor="pink">
      <Alert status={message.status}>
        <VStack>
        <AlertTitle mr={2}>{message.title}</AlertTitle>
        <AlertDescription>{message.description}</AlertDescription>
        </VStack>
        <CloseButton position='absolute' right='8px' top='8px' onClick={closeMe} />
      </Alert>
    </Box>
  );
};
